...
let shieldCooldown = 0; // New cooldown timer
let mixer;
...
function setupEventListeners() {
    ...
    window.addEventListener('keydown', e => {
        keys[e.code] = true;
        // Activate shield only if not active and cooldown is 0
...
function createPlayer() {
    // Simple player model as fallback
...
function updateAsteroids() {
    for (let i = asteroids.length-1; i >= 0; i--) {
        ...
        if (player.position.distanceTo(ast.position) < 1) {
            handleDamage();
            scene.remove(ast);
            asteroids.splice(i,1);
        }
...
function updateBullets() {
    for (let i = bullets.length-1; i >= 0; i--) {
        ...
        for (let j = asteroids.length-1; j >= 0; j--) {
            let a = asteroids[j];
            if (b.position.distanceTo(a.position) < 0.8) {
                scene.remove(a);
